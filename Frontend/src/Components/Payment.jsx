    ...
    <>
      ...
      <div className="flex gap-2 justify-center">
        ...
        <div className="flex flex-col p-4 w-[50%]">
          <h1 className="bg-blue-500 p-2 font-semibold text-lg text-white w-full rounded-lg">Delivery Address</h1>
          <select
            name="address"
            id="address-select"
          ...
          >
            ...
            {address.map((item) => (
              <option key={item.id} value={item.id}>
                <span className="font-semibold">{item.name}</span> {item.address}, {item.locality}, {item.city}-{item.pincode}
              </option>
            ))}
          </select>
          <div>
            <h1 className="bg-blue-500 p-2 font-semibold text-lg text-white w-full rounded-lg mt-2">Order Summary</h1>
            ...
            <button
                className={`items-center ${
                  cart.length > 0 ? "bg-orange-500" : "bg-gray-400 cursor-not-allowed"
          ...
          {payment?(
            ...
            <h1 className="bg-blue-500 p-2 font-semibold text-lg text-white w-full rounded-lg">Payment</h1>
          )}
        </div>
        <div className="w-full md:w-1/3 bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="text-lg font-semibold space-y-2">
            <p className="flex justify-between">
              <span>Total Amount:</span> ₹{subtotalAmount}
            </p>
            <p className="flex justify-between">
